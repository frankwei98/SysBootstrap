...
- KTD3. Manage SSH settings in `/etc/ssh/sshd_config.d/00-sys-bootstrap.conf`, but only when the main config has an effective early Include compatible with that strategy; otherwise fail before mutation. Scalar authentication directives are owned by this early drop-in. Port handling is explicitly additive during prepare so old effective ports remain available beside the new port. During finalize, snapshot and comment explicit legacy `Port` directives before validating the exclusive replacement listener; any finalization failure restores those files byte for byte. Validate global settings and representative root/target-user contexts with `sshd -T`/`-C`.
...
3. Missing/late Include, a trailing `Match`, or conflicting conditional auth fails before destructive finalization; explicit legacy Port values are migrated transactionally and restored if finalization fails.
